...
bool DTB::generate(VMContext& ctx) {
    ...
    // ── Always-present virtio devices ────────────────────────────────────────
    // Console and block are unconditional; their MMIO handlers return safe
    // defaults (DeviceId=0 suppresses probing) when the device object is null.
    struct VirtioEntry { const char* name; uint64_t gpa; uint32_t spi; };
    static constexpr VirtioEntry kVirtioAlways[] = {
        {"virtio_mmio@a000000", kGpaVirtioConsole, kSpiConsole},
        {"virtio_mmio@a000200", kGpaVirtioBlock,   kSpiBlock  },
    };
    for (const auto& dev : kVirtioAlways) {
    ...
    // ── Optional virtio devices ───────────────────────────────────────────────
    // Only emitted when the corresponding device object exists so the kernel
    // never probes an MMIO region that would return a zero magic value.
    if (ctx.net()) {
        b.begin_node("virtio_mmio@a000400");
        b.prop_strlist("compatible", {"virtio,mmio"});
        b.prop_cells  ("reg",        {0, uint32_t(kGpaVirtioNet), 0, uint32_t(kGpaVirtioStride)});
        b.prop_cells  ("interrupts", {0, kSpiNet, 1});
        b.prop_empty  ("dma-coherent");
        b.end_node();
    }
    ...
    logger::Info("[dtb] %zu bytes at GPA 0x%llx\n",
