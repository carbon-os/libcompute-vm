    ...
    , height_(height)
{
    ...
    logger::Info("[gpu] virtio-gpu 2D initialized (%ux%u)\n", width_, height_);
}
...
void Gpu::handle_command(const uint8_t* req, uint32_t req_len,
    ...
    case VIRTIO_GPU_CMD_RESOURCE_CREATE_2D: {
        ...
        logger::Info("[gpu] resource_create id=%u %ux%u fmt=%u\n",
    ...
    case VIRTIO_GPU_CMD_SET_SCANOUT: {
        ...
        if (cmd.scanout_id == 0) {
            ...
            logger::Info("[gpu] scanout 0 → resource %u\n", cmd.resource_id);
        }
        ok_nodata();
        break;
    }
    ...
    case VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING: {
        ...
        logger::Info("[gpu] resource %u attached %u backing pages\n",
    ...
    default:
        logger::Warn("[gpu] unknown virtio cmd 0x%x\n", hdr.type);
        ...
        case Reg::Status: {
                ...
                !(prev & VIRTIO_STATUS_FEATURES_OK))
            {
                if (driver_features_ & ~device_features_) {
                    ...
                    logger::Warn("[gpu] FEATURES_OK refused: driver=0x%llx device=0x%llx\n",
