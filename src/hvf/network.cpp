...
namespace compute::vm::internal::hvf::network {
...
struct ConnKeyHash {
    size_t operator()(const ConnKey& k) const noexcept {
        ...
        h = h * 2654435761u ^ (static_cast<uint32_t>(k.sp) << 16 | k.dp);
        return h;
    }
};
...
struct Stack::Impl {
    ...
    std::mutex        mtx;
    std::unordered_map<ConnKey, std::shared_ptr<TcpConn>, ConnKeyHash> conns;
    ...
    void handle_icmp(const uint8_t* eth_src, const Ip4Hdr* ip,
        ...
        if (payload[0] != 8) return; // only echo requests
        ...
        emit_ip(eth_src, ntohl(ip->dst), ntohl(ip->src),
                IP_ICMP, rep.data(), rep.size());
    }
    ...
    void handle_dhcp(const uint8_t* eth_src, const uint8_t* payload, size_t len) {
        ...
        uint8_t reply[548]{};
        reply[0] = 2; reply[1] = 1; reply[2] = 6; // BOOTREPLY, Ethernet, hlen=6
        std::memcpy(reply + 4, &xid, 4);
        const uint32_t yip = htonl(cfg.guest_ip);
        const uint32_t sip = htonl(cfg.gw_ip);
        std::memcpy(reply + 16, &yip, 4); // yiaddr
        std::memcpy(reply + 20, &sip, 4); // siaddr
        ...
        opt8 (53, msg_type == 1 ? 2 : 5); // OFFER or ACK
        opt32(54, cfg.gw_ip);              // server identifier
        opt32(51, 86400);                  // lease time: 24h
        opt32(1,  0xFFFFFF00u);            // subnet mask
        opt32(3,  cfg.gw_ip);             // router
        opt32(6,  cfg.dns_ip);            // DNS
        reply[p++] = 255;                  // END
        ...
        logger::Info("[net] DHCP %s → %u.%u.%u.%u\n",
                     msg_type == 1 ? "OFFER" : "ACK",
                     (cfg.guest_ip >> 24) & 0xff, (cfg.guest_ip >> 16) & 0xff,
                     (cfg.guest_ip >>  8) & 0xff,  cfg.guest_ip        & 0xff);
    }
    ...
    void handle_udp(const uint8_t* eth_src, const Ip4Hdr* ip,
        ...
        // Generic UDP: punch through to the real host network
        const int sock = ::socket(AF_INET, SOCK_DGRAM, 0);
        if (sock < 0) return;
        struct timeval tv{2, 0};
        ::setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
        ...
        sa.sin_addr.s_addr = ip->dst; // already network byte order
    ...
    void launch_io_thread(std::shared_ptr<TcpConn> conn, ConnKey key) {
        std::thread([this, conn = std::move(conn), key]() mutable {
            ...
            // Send FIN to guest if we weren't told to stop by the guest side
            if (!conn->stop.load(std::memory_order_relaxed)) {
                const uint32_t seq = conn->h_nxt.fetch_add(1, std::memory_order_relaxed);
                send_tcp(conn->guest_mac,
                         conn->stk_src_ip,   conn->stk_src_port,
                         conn->stk_dst_ip,   conn->stk_dst_port,
    ...
    void handle_tcp(const uint8_t* eth_src, const Ip4Hdr* ip,
        ...
        if ((flags & TF_SYN) && !(flags & TF_ACK)) {
            ...
            const uint32_t dst_ip_n = ip->dst; // network byte order for connect()
            ...
            std::thread([this, conn, key, dst_ip_n, dp]() {
                ...
                if (::poll(&pfd, 1, 5000) > 0 && !(pfd.revents & POLLERR)) {
                    ::fcntl(fd, F_SETFL, ::fcntl(fd, F_GETFL) & ~O_NONBLOCK);
                    conn->fd = fd;
                    conn->state.store(TcpConn::State::SYNACK_SENT,
                                      std::memory_order_relaxed);
                    const uint32_t isn = conn->h_nxt.fetch_add(1,
                                            std::memory_order_relaxed);
                    send_tcp(conn->guest_mac,
                             conn->stk_src_ip, conn->stk_src_port,
                             conn->stk_dst_ip, conn->stk_dst_port,
                    ...
                    ::close(fd);
                    logger::Warn("[net] TCP connect to port %u failed\n", dp);
                    std::lock_guard<std::mutex> lk(mtx);
                    conns.erase(key);
                }
            }).detach();
        ...
        if (conn->state.load() == TcpConn::State::SYNACK_SENT) {
            if (flags & TF_ACK) {
                conn->state.store(TcpConn::State::ESTABLISHED,
                                  std::memory_order_relaxed);
        ...
        const uint32_t ack_out = conn->g_nxt.load();
        const uint32_t h_seq   = should_fin
            ? conn->h_nxt.fetch_add(1, std::memory_order_relaxed)
            : conn->h_nxt.load();
        ...
        if (should_fin) {
            send_tcp(conn->guest_mac,
                     conn->stk_src_ip, conn->stk_src_port,
                     conn->stk_dst_ip, conn->stk_dst_port,
                     h_seq, ack_out, TF_FIN | TF_ACK);
        } else if (should_ack) {
            send_tcp(conn->guest_mac,
                     conn->stk_src_ip, conn->stk_src_port,
                     conn->stk_dst_ip, conn->stk_dst_port,
                     h_seq, ack_out, TF_ACK);
        }
    }
    ...
    void inbound_fwd(int host_fd, uint16_t guest_port) {
        ...
        const uint32_t our_isn = 0x50000000u | (static_cast<uint32_t>(eport) << 8u);
        ...
        // Retransmit SYN up to 5 times waiting for the guest to respond
        ...
        logger::Warn("[net] port forward to guest:%u timed out\n", guest_port);
        std::lock_guard<std::mutex> lk(mtx);
        conns.erase(key);
    }
    ...
    void port_fwd_listen(PortForward pf) {
        ...
        if (::bind(lfd, reinterpret_cast<sockaddr*>(&la), sizeof(la)) < 0 ||
            ::listen(lfd, 8) < 0) {
            logger::Error("[net] port forward bind on host:%u failed: %s\n",
                          pf.host_port, strerror(errno));
            ::close(lfd);
            return;
        }

        logger::Info("[net] listening host:%u → guest:%u\n",
