...
bool load_file(std::string_view path, void* dst,
    ...
    if (!f) {
        logger::Error("[loader] cannot open %s: %s\n", p.c_str(), strerror(errno));
        return false;
    }
    ...
    if (sz <= 0 || static_cast<std::size_t>(sz) > max_size) {
        logger::Error("[loader] %s: size %ld out of range (max=%zu)\n", p.c_str(), sz, max_size);
        std::fclose(f);
        return false;
    }
    ...
    if (std::fread(dst, 1, usz, f) != usz) {
        logger::Error("[loader] fread failed for %s: %s\n", p.c_str(), strerror(errno));
        std::fclose(f);
        return false;
    }
...
bool Loader::load_kernel(VMContext& ctx, std::string_view path) {
    ...
    if (!dst) {
        logger::Error("[loader] kernel load GPA 0x%llx not mapped\n",
    ...
    if (hdr->magic != kArm64Magic) {
        logger::Error("[loader] bad arm64 magic: expected 0x%08x got 0x%08x\n",
                      kArm64Magic, hdr->magic);
        return false;
    }

    ctx.set_kernel_entry(kGpaKernelLoad + hdr->text_offset);
    logger::Info("[loader] kernel: %zu bytes  entry GPA 0x%llx\n",
...
bool Loader::load_initrd(VMContext& ctx, std::string_view path) {
    ...
    if (!dst) {
        logger::Error("[loader] initrd load GPA 0x%llx not mapped\n",
    ...
    logger::Info("[loader] initrd: %zu bytes at GPA 0x%llx\n",
