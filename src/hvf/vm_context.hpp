...
struct Pl011State {
    ...
    uint32_t lcr  {0};
    uint32_t imsc {0};
    int      tx_fd{-1};  // host-side write end of the serial pipe; -1 = logger fallback
};
...
public:
    ...
    [[nodiscard]] bool create_console(int rx_fd, int tx_fd);
    ...
    void set_pl011_tx_fd(int fd)                   noexcept { pl011_.tx_fd = fd; }
...
#define HV_CHECK(expr, msg) \
    do { if (!::compute::vm::internal::hvf::hv_check((expr), (msg))) return false; } while (0)
