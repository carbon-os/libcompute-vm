...
namespace compute::vm::internal::hvf { bool psci_dispatch(VCpu& vcpu); }
...
bool handle_mmio(VCpu& vcpu) {
        ...
        if (wnr) {
            ...
            switch (off) {
                case kPl011Dr: {
                    ...
                    // Route to the serial pipe if wired up, otherwise fall back to logger
                    if (pl011.tx_fd >= 0)
                        ::write(pl011.tx_fd, &c, 1);
                    else
                        logger::write(&c, 1);
                    break;
                }
        ...
        if (!wnr) {
            ...
            if (ipa >= kGpaGicRedist && ipa < kGpaGicRedist + kGpaGicRedistSize) {
                ...
                if (frame_off == kGicPidr2Offset)
                    ...
                    logger::Warn("[exit] unhandled GICR read IPA 0x%llx\n",
                                 static_cast<unsigned long long>(ipa));
            } else {
                logger::Warn("[exit] unexpected GICD read IPA 0x%llx\n",
                             static_cast<unsigned long long>(ipa));
            }
            hv_vcpu_set_reg(vcpu.handle(), xreg_id(srt), val);
        }
    ...
    logger::Warn("[exit] unhandled MMIO %s IPA 0x%llx len=%u\n",
                 wnr ? "W" : "R", static_cast<unsigned long long>(ipa), len);
    advance_pc(vcpu);
    return true;
}
    ...
    switch (ex->reason) {
        ...
        case HV_EXIT_REASON_EXCEPTION: {
            ...
            switch (ec) {
                ...
                default:
                    logger::Error("[exit] vcpu%u unhandled EC=0x%02x ESR=0x%016llx\n",
                                  vcpu.index(), ec, static_cast<unsigned long long>(esr));
                    advance_pc(vcpu);
                    return true;
            }
        }
        default:
            logger::Error("[exit] vcpu%u unknown exit reason 0x%x\n",
