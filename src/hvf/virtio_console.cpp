...
Console::Console(VMContext& ctx, uint64_t /*gpa*/, int rx_fd, int tx_fd) noexcept
    : ctx_(ctx), rx_fd_(rx_fd), tx_fd_(tx_fd), features_(kVirtioF_Version1)
{
    if (rx_fd_ >= 0) {
...
void Console::rx_thread_func() {
    struct pollfd pfd {rx_fd_, POLLIN, 0};
...
void Console::process_tx() {
    ...
    while (q.last_avail != avail->idx) {
        ...
        do {
            if (void* buf = ctx_.gpa_to_hva(desc[cur].addr); buf && desc[cur].len) {
                if (tx_fd_ >= 0) {
                    ::write(tx_fd_, buf, desc[cur].len);
                }
                wrote += desc[cur].len;
            }
...
void Console::write(uint32_t off, uint32_t val, uint32_t) {
    std::lock_guard<std::mutex> lk(mtx_);
